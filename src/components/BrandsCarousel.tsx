...
export const BrandsCarousel: React.FC = () => {
  ...
  // Duplicate the list so it scrolls seamlessly in marquee
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full overflow-visible py-6 group/marquee">
        ...
        <div className="flex gap-6 sm:gap-10 animate-[marqueeScroll_35s_linear_infinite] group-hover/marquee:[animation-play-state:paused] w-max items-center">
          {doubledLogos.map((brand, idx) => (
            <div
              key={`${brand.id}-${idx}`}
              className="group relative flex flex-col items-center justify-center w-[180px] sm:w-[220px] h-[95px] sm:h-[115px] px-6 py-4 rounded-2xl bg-[#060B18] border border-white/10 transition-all duration-300 hover:scale-115 hover:z-30 hover:border-[#3870E0] hover:bg-[#0A1636] hover:shadow-[0_0_35px_rgba(59,130,246,0.7)] cursor-pointer"
            >
              {/* Logo with grayscale by default, substantially scales up and illuminates on hover */}
              <div className="w-full h-full flex items-center justify-center overflow-visible">
                <img
                  src={brand.logo}
                  alt={brand.name}
                  className="max-h-[50px] sm:max-h-[62px] max-w-[135px] sm:max-w-[165px] object-contain filter grayscale opacity-45 transition-all duration-300 group-hover:filter-none group-hover:opacity-100 group-hover:scale-135 sm:group-hover:scale-145 group-hover:drop-shadow-[0_0_20px_rgba(59,130,246,0.95)]"
                  loading="lazy"
                />
              </div>
